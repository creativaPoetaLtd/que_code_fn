      ...
      <React.Fragment key={index}>
        <div className="flex items-center transition-all duration-500">
          ...
          {index < steps.length - 1 && (
            <div
              className={`w-24 h-1 ml-4 transition-all duration-1500
                            ${
                              currentStep > index
                                ? "bg-[#00B512]"
                                : "bg-gray-200"
...
const UserRegister: React.FC = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // API returns: { message, data, emailSent }
      const message = response?.message || "User registered successfully.";
      const emailSent = response?.emailSent;
      setApiSuccess(
        emailSent
          ? message
          : `${message} However, we couldn't send the verification email. Please contact support.`
      );
      // Redirect to OTP page with email and userId if needed
      // Commented out to keep form visible after registration
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen">
      <div className="flex flex-col justify-center lg:w-1/2 w-full mt-8 md:px-32 px-6">
        ...
        {/* Show API error or success message above the form */}
        {apiError && (
        ...
        <form
          className="space-y-6 mt-8 transition-opacity duration-500"
          onSubmit={handleSubmit(onSubmit)}
        >
          <div key={`step-${currentStep}`}>{stepContent[currentStep]}</div>
          ...
          <p className="mt-6 text-sm text-center">
            ...
            <Link href="/auth/login" className="text-[#00B512] hover:underline">
              Sign in
            </Link>
          </p>
        </form>
