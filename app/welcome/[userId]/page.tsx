...
const WelcomeProfilePage = () => {
  ...
  // Get contacts to check if current user is already a friend
  const token = getToken();
  const { data: contactsData } = useGetContactsQuery(token || "", {
  ...
  // Check if current profile user is already a contact
  useEffect(() => {
    if (contactsData?.data && userId && currentUserId) {
      const isContact = contactsData.data.some(
        (contact: any) => contact.contactUser?.id === userId
      );
      setIsAlreadyContact(isContact);
    }
  }, [contactsData, userId, currentUserId]);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        const profileUrl = `${baseUrl}/profiles?userId=${encodeURIComponent(
          userId
        )}`;
        ...
        if (userRes.status === "fulfilled") {
          ...
          if (userRes.reason.response) {
            console.error(
              "[Welcome] User response status:",
              userRes.reason.response.status
            );
            console.error(
              "[Welcome] User response data:",
              userRes.reason.response.data
            );
          }
        }
        ...
        if (profileRes.status === "fulfilled") {
          ...
          console.error(
            "[Welcome] Failed to fetch profile data:",
            profileRes.reason
          );
          if (profileRes.reason.response) {
            console.error(
              "[Welcome] Profile response status:",
              profileRes.reason.response.status
            );
            console.error(
              "[Welcome] Profile response data:",
              profileRes.reason.response.data
            );
          }
        ...
        setError(
          `Could not load profile data for user ID: ${userId.substring(
            0,
            8
          )}...`
        );
  ...
  const handleAddFriend = async () => {
    ...
    // Construct the profile URL for the current user
    const profileUrl = `${window.location.origin}/welcome/${userId}`;
    ...
    try {
      ...
      toast({
        ...
        description:
          result.message || `Invitation sent successfully to ${user.name}`,
      });
    } catch (error: any) {
  ...
  const handleSendMessage = () => {
    // Navigate to chat page for this contact
      ...
      <>
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#013f47] via-[#025059] to-[#01363d] flex items-center justify-center px-4 relative overflow-hidden">
          ...
          <div className="absolute inset-0 overflow-hidden">
            ...
            {[DollarSign, CreditCard, Coins, Banknote, Wallet].map(
              (Icon, index) => (
                <div
                  key={index}
                  className={`absolute animate-bounce delay-${
                    (index + 1) * 1000
                  }`}
                  style={{
          ...
          <div className="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 p-8 md:p-12 flex flex-col items-center max-w-md w-full relative z-10">
            ...
            <div className="text-center space-y-3">
              <p className="text-[#00313A] font-semibold text-lg">
                Loading profile...
              </p>
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#013f47] via-[#025059] to-[#01363d] relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          {[
            ...
            <div
              key={index}
              className={`absolute ${position} animate-bounce delay-${delay}`}
            >
              ...
              <div className="flex flex-col items-center mt-6 mb-8">
                ...
                <div className="text-center w-full">
                  <h1 className="text-3xl xl:text-4xl font-bold text-[#00313A] mb-3 leading-tight">
                    <b>{user.name}</b>
                    <span className="inline-block ml-3 text-2xl xl:text-3xl">
                      💸
                    </span>
                  </h1>
                ...
                <div className="w-full mt-6 space-y-6">
                  ...
                  {user.profileType && user.showProfileTypeOnWelcome && (
                    <div className="flex justify-center animate-fade-in">
                      <div
                        className={`px-6 py-3 rounded-full text-sm font-bold shadow-lg transform hover:scale-105 transition-all duration-300 animate-pulse ${
                    ...
                    user.showLocationOnWelcome && (
                      <div className="bg-gradient-to-br from-white via-[#f8fffa] to-[#f0fff4] rounded-2xl p-6 border-2 border-[#00B512]/10 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] animate-fade-in">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <h3 className="text-lg font-bold text-[#00313A]">
                            Location Information
                          </h3>
                        </div>
                          ...
                          {user.province && (
                            <div className="bg-white/60 rounded-xl p-3 border border-[#00B512]/10 hover:bg-white/80 transition-all duration-200">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-[#00B512] text-sm">
                                  🏛️
                                </span>
                                <span className="text-[#00B512] font-semibold text-sm">
                                  Province
                                </span>
                              </div>
                              <span className="text-[#00313A] font-medium">
                                {user.province}
                              </span>
                            </div>
                          )}
                    ...
                    <div className="bg-gradient-to-br from-white via-[#f8fffa] to-[#f0fff4] rounded-2xl p-6 border-2 border-[#00B512]/10 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] animate-fade-in">
                      ...
                      <div className="bg-white/60 rounded-xl p-4 border border-[#00B512]/10 hover:bg-white/80 transition-all duration-200">
                        <div className="flex items-center gap-3">
                          <span className="text-[#00B512] font-bold text-lg">
                            TIN
                          </span>
                        ...
                        <p className="text-[#00313A]/70 text-sm mt-2">
                  ...
                  <div className="flex justify-center gap-4 w-full">
                    <Dialog>
                      <DialogTrigger asChild>
                        <CustomButton
                          variant="default"
                          className="px-6 py-3 rounded-xl font-bold shadow-lg bg-gradient-to-r from-[#00B512] to-[#1fd331] text-white"
                        >
                          Send Money
                        </CustomButton>
                      </DialogTrigger>
                    ...
                    {isLoggedIn && currentUserId !== userId && (
                      <CustomButton
                        variant="outline"
                        className="px-6 py-3 border-2 border-[#00B512] text-[#00B512] rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 hover:bg-[#00B512] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                        onClick={
                          isAlreadyContact ? handleSendMessage : handleAddFriend
                        }
                        disabled={isInviting}
                      >
                    ...
                    {!isLoggedIn && (
                      <Dialog>
                        <DialogTrigger asChild>
                          <CustomButton
                            variant="outline"
                            className="px-6 py-3 rounded-xl font-bold shadow-lg border-[#00B512] text-[#00B512]"
                          >
                            Contact
                          </CustomButton>
                        </DialogTrigger>
                  ...
                  {isLoggedIn ? (
                    <div className="w-full max-w-xs flex flex-col items-center gap-3 bg-gradient-to-br from-[#f0fff4] via-[#e6f9f0] to-[#f6fff9] rounded-2xl shadow-lg p-5 mt-2 border border-[#00B512]/10">
                      <div className="text-center mb-2">
                        <span className="block text-lg font-bold text-[#00B512] drop-shadow-sm">
                          Welcome back!
                        </span>
                        <span className="block text-sm text-[#00313A]/70 mt-1">
                          You're all set to send money and connect 🎉
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="w-full max-w-xs flex flex-col items-center gap-3 bg-gradient-to-br from-[#f0fff4] via-[#e6f9f0] to-[#f6fff9] rounded-2xl shadow-lg p-5 mt-2 border border-[#00B512]/10">
                      <div className="text-center mb-2">
                        <span className="block text-lg font-bold text-[#00B512] drop-shadow-sm">
                          Join us now or sign in!
                        </span>
                        <span className="block text-sm text-[#00313A]/70 mt-1">
                          Enjoy secure, fast, and fun money transfers 🚀
                        </span>
                      </div>
            ...
            <div className="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 p-6 relative transform hover:scale-[1.02] transition-all duration-300">
              ...
              <div className="w-full mt-4 space-y-4">
                ...
                {user.profileType && user.showProfileTypeOnWelcome && (
                  <div className="flex justify-center animate-fade-in">
                    <div
                      className={`px-4 py-2 rounded-full text-xs font-bold shadow-md transform hover:scale-105 transition-all duration-300 animate-pulse ${
                        user.profileType === "organization"
                          ? "bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 text-white shadow-purple-500/25 hover:shadow-purple-500/40"
                          : "bg-gradient-to-r from-[#00B512] via-[#1fd331] to-[#00B512] text-white shadow-[#00B512]/25 hover:shadow-[#00B512]/40"
                      }`}
                  ...
                  user.showLocationOnWelcome && (
                    <div className="bg-gradient-to-br from-white via-[#f8fffa] to-[#f0fff4] rounded-xl p-4 border-2 border-[#00B512]/10 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] animate-fade-in">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-sm font-bold text-[#00313A]">
                          Location
                        </h3>
                      </div>
                      <div className="space-y-2">
                        {user.province && (
                          <div className="bg-white/60 rounded-lg p-2 border border-[#00B512]/10 hover:bg-white/80 transition-all duration-200">
                            <div className="flex items-center gap-1 mb-1">
                              ...
                              <span className="text-[#00B512] font-semibold text-xs">
                                Province
                              </span>
                            </div>
                            <span className="text-[#00313A] font-medium text-xs">
                              {user.province}
                            </span>
                          </div>
                        )}
                  ...
                  <div className="bg-gradient-to-br from-white via-[#f8fffa] to-[#f0fff4] rounded-xl p-4 border-2 border-[#00B512]/10 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] animate-fade-in">
                    ...
                    <div className="bg-white/60 rounded-lg p-3 border border-[#00B512]/10 hover:bg-white/80 transition-all duration-200">
                      <div className="flex items-center gap-2">
                        <span className="text-[#00B512] font-bold text-sm">
                          TIN
                        </span>
                      ...
                      <p className="text-[#00313A]/70 text-xs mt-1">
                        Tax ID Number
                      </p>
                    </div>
                  </div>
              ...
              {isLoggedIn ? (
                  ...
                  <div className="flex flex-col gap-3">
                    <Dialog>
                      <DialogTrigger asChild>
                        <CustomButton
                          variant="default"
                          className="w-full h-12 bg-gradient-to-r from-[#00B512] to-[#1fd331] text-white rounded-xl font-bold shadow-lg"
                        >
                          Send Money
                        </CustomButton>
                      </DialogTrigger>
                    ...
                    {currentUserId !== userId && (
                      <CustomButton
                        variant="outline"
                        className="w-full h-12 border-2 border-[#00B512] text-[#00B512] rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 hover:bg-[#00B512] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                        onClick={
                          isAlreadyContact ? handleSendMessage : handleAddFriend
                        }
                        disabled={isInviting}
                ...
                <div className="w-full space-y-4">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <CustomButton
                          variant="outline"
                          className="w-full h-12 border-2 border-[#00B512] text-[#00B512] rounded-xl font-bold shadow-lg"
                        >
                          Contact
                        </CustomButton>
                      </DialogTrigger>
                  ...
                  <div className="w-full flex flex-col items-center gap-3 bg-gradient-to-br from-[#f0fff4] via-[#e6f9f0] to-[#f6fff9] rounded-2xl shadow-lg p-4 mt-2 border border-[#00B512]/10">
                    <div className="text-center mb-2">
                      <span className="block text-base font-bold text-[#00B512] drop-shadow-sm">
                        Join us now or sign in!
                      </span>
                      <span className="block text-xs text-[#00313A]/70 mt-1">
                        Enjoy secure, fast, and fun money transfers 🚀
                      </span>
                    </div>
                    <div className="flex gap-3 w-full">
              ...
              {isHydrated && isLoggedIn && (
                <div className="mt-6 text-center space-y-2">
                  ...
                  <p className="text-xs text-[#00313A] font-medium">
                    QiewCode Made with{" "}
                    <Heart className="w-3 h-3 text-[#00B512] inline mx-1 animate-pulse" />
                    for you
                  </p>
                </div>
              )}
        ...
        <footer className="bg-[#00313A] text-white py-8 mt-12">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                <h3 className="text-lg font-bold mb-4 text-[#00B512]">
                  QiewCode
                </h3>
                <p className="text-sm text-gray-300">
                  Secure, fast, and reliable money transfer platform designed
                  for modern users.
                </p>
              </div>
              <div>
                <h4 className="text-md font-semibold mb-4 text-[#00B512]">
                  Quick Links
                </h4>
                <ul className="space-y-2 text-sm text-gray-300">
                  <li>
                    <a
                      href="/home"
                      className="hover:text-[#00B512] transition-colors"
                    >
                      Home
                    </a>
                  </li>
            ...
            <div className="border-t border-gray-600 mt-8 pt-8 text-center">
              ...
              <p className="text-sm text-gray-400">
                © 2024 QiewCode. All rights reserved. Made with ❤️ for secure
                money transfers.
              </p>
            </div>
          </div>
