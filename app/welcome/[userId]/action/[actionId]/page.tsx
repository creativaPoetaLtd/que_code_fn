...
function VoteContent({
  ...
  return (
    <>
      {/* Hero: image with overlay (left) + meta/controls (right) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-6">
        {/* Left: image with title + badges overlaid */}
        ...
        {/* Right: description + quick-pick + search + filters */}
        <div className="flex flex-col gap-4">
          ...
          {activeSubActions.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {activeSubActions.slice(0, 4).map((c: SubAction) => (
                <button
                  key={c.id}
              ...
              return (
                <div
                  ...
                  className={`rounded-xl border overflow-hidden transition-colors ${
                    isSelected ? 'bg-[#1a3a5c] border-[#3b82f6]' : 'bg-[#111927] border-[#1e2d40] hover:border-[#2a3d54]'
                  }`}
                >
                  {/* Avatar + name row */}
                  <div className="flex items-center gap-3 p-4 pb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {badge && (
                        <span className="inline-block bg-[#1a2c3d] text-[#5b8aaa] rounded-md px-2 py-0.5 text-xs mt-1">{badge}</span>
                      )}
                    </div>
                  </div>
                  ...
                  {hasStats && (
                    <div className="mx-4 mb-3 bg-[#0d1525] rounded-lg flex overflow-hidden">
                      <div className="flex-1 p-2.5 text-center border-r border-[#1e2d40]">
                        ...
                        <p className="text-[#f0f4f8] font-bold text-sm">{votes !== undefined ? Number(votes).toLocaleString() : '—'}</p>
                      </div>
                      <div className="flex-1 p-2.5 text-center border-r border-[#1e2d40]">
                        <p className="text-[#4a6278] text-xs mb-0.5">Rank</p>
                        <p className="text-[#f0f4f8] font-bold text-sm">{rank !== undefined ? `#${rank}` : '—'}</p>
                      </div>
                  ...
                  <div className="flex gap-2 px-4 pb-4">
                    ...
                    >
                      {purchasing[candidate.id] ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Voting...</> : 'Vote now'}
                    </button>
                  </div>

                  {/* "More about this candidate" expandable */}
                  <button
      ...
      {selectedCandidate && (
        <div className="fixed bottom-0 left-0 right-0 bg-[#0f1924] border-t border-[#1e2d40] px-4 sm:px-6 py-4 z-30">
          <div className="max-w-[960px] mx-auto flex items-center justify-between gap-6">
            ...
            >
              {purchasing[selectedCandidate.id] ? <><Loader2 className="w-4 h-4 animate-spin" />Voting...</> : 'Vote'}
            </button>
          </div>
        </div>
      )}
