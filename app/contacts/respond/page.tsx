...
import { useRespondToInvitationMutation } from "@/states/contactInvitationSlice";
  ...
  const handleInvitationResponse = async (details: InvitationDetails) => {
    ...
    try {
      ...
      const result = await respondToInvitation({
        invitationId: parseInt(details.contactId),
        action: details.action === "reject" ? "decline" : "accept",
        token: authToken,
      }).unwrap();

      // Invitation responded successfully
          ...
          <Card className="w-full max-w-md mx-auto">
            <CardContent className="flex flex-col items-center justify-center p-8">
              ...
              <p className="text-gray-600 text-center">
                {invitationDetails
                  ? `${
                      getActionText(invitationDetails.action)
                        .charAt(0)
                        .toUpperCase() +
                      getActionText(invitationDetails.action).slice(1)
            ...
            <CardContent className="flex flex-col items-center justify-center p-8">
              ...
              <h2 className="text-xl font-semibold mb-2">
                ...
                {getActionPastTense(invitationDetails?.action || "")
                  .charAt(0)
                  .toUpperCase() +
                  getActionPastTense(invitationDetails?.action || "").slice(1)}
              </h2>
              <p className="text-gray-600 text-center mb-6">
                {isAccepted
                  ? `You have successfully accepted the contact invitation${
                      inviterName ? ` from ${inviterName}` : ""
                    }. They have been added to your contacts.`
                  : `You have rejected the contact invitation${
                      inviterName ? ` from ${inviterName}` : ""
                    }.`}
              </p>
