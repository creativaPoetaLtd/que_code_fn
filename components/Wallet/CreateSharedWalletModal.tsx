...
import { useCreateSharedWalletMutation } from "@/states/sharedWalletSlice"
...
/**
 * Standalone creation flow - no group/chat required. This is the Wallet page's
 * entry point; components/chat/create-shared-wallet-modal.tsx is the separate
 * chat-triggered flow that creates a group with a shared wallet attached.
 */
export default function CreateSharedWalletModal({ isOpen, onClose }: CreateSharedWalletModalProps) {
    ...
    const [createSharedWallet, { isLoading: isCreating }] = useCreateSharedWalletMutation()
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast({ title: "Shared wallet created", description: `${walletName.trim()} is ready — RWF 0 to start.` })
            onClose()
        } catch (error: any) {
                            ...
                            <div className="space-y-3">
                                ...
                                <RadioGroup value={withdrawalPolicy} onValueChange={(value: WithdrawalPolicy) => setWithdrawalPolicy(value)}>
                                    <div className="flex items-start space-x-2">
                                        <RadioGroupItem value="free" id="standalone-policy-free" className="mt-0.5" />
                                        <Label htmlFor="standalone-policy-free" className="text-sm">
                                    ...
                                    <div className="flex items-start space-x-2">
                                        <RadioGroupItem value="approval" id="standalone-policy-approval" className="mt-0.5" />
                                        <Label htmlFor="standalone-policy-approval" className="text-sm">
