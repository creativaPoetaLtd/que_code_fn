...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({ 
  ...
  return (
    <div className="py-8 text-center">
      <div className="inline-flex items-center gap-2 dark:text-gray-300">
...
export const ErrorMessage: React.FC<ErrorMessageProps> = ({ message, onRetry }) => {
  return (
    <div className="py-8 text-center">
      <div className="inline-flex flex-col items-center gap-2 text-red-500 dark:text-red-400">
        ...
        {onRetry && (
          <button 
            onClick={onRetry}
            className="mt-2 px-4 py-2 text-sm border border-gray-300 dark:border-darkBorder-light rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 dark:text-gray-300 transition-colors"
          >
            Try Again
          </button>
        )}
...
export const SuccessMessage: React.FC<SuccessMessageProps> = ({ message, details }) => {
  return (
    <div className="mb-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
      ...
      {details && (
        <p className="text-sm text-green-700 dark:text-green-300 mt-2">{details}</p>
      )}
    </div>
  );
};
...
export const VerificationCard: React.FC<VerificationCardProps> = ({ 
  ...
  const statusConfig = {
    verified: {
      ...
      badgeColor: "bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300 border-green-200 dark:border-green-800",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
    ...
    <div className={`flex items-center justify-between p-3 ${config.bgColor} rounded-lg`}>
      ...
      <div className="flex items-center gap-2">
        ...
        {config.showButton && onAction && (
          <button
            onClick={onAction}
            className="px-3 py-1 text-sm bg-[#00B512] hover:bg-[#009E10] dark:bg-green-700 dark:hover:bg-green-600 text-white rounded transition-colors"
          >
            Verify now
          </button>
        )}
