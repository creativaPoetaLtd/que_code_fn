...
export default function ContactRequestModal({
  ...
  const handleResponse = async (
    ...
    try {
      const result = await respondToInvitation({
        invitationId: parseInt(contactId),
        action: action === "reject" ? "decline" : "accept",
        token,
      }).unwrap();
  ...
  // Extract data
  const receivedInvitations = pendingInvitations?.data || [];
  const completedContacts = acceptedContacts?.data || [];

  // Loading states
  const isLoading =
    isPendingLoading || (activeFilter === "completed" && isAcceptedLoading);

  // Error states
  const hasError =
    pendingError || (activeFilter === "completed" && acceptedError);
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-8 text-center">
            ...
            <p className="text-gray-500">
              Please log in to view contact requests
            </p>
          </div>
          <DialogFooter>
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              Connected:{" "}
                              {new Date(
                                contact.respondedAt || contact.invitedAt
                              ).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
