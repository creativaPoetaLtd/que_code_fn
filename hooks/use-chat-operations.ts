...
export function useChatOperations(): UseChatOperationsReturn {
    ...
    const [createOrGetDMChat, { isLoading: isCreatingDMChat }] = useCreateOrGetDMChatMutation()
    const [sendMessage, { isLoading: isSendingMessage }] = useSendMessageMutation()
    const [markAsRead] = useMarkMessagesAsReadMutation()
    const [createGroupChat, { isLoading: isCreatingGroup }] = useCreateGroupChatMutation()
    const [joinGroupChat, { isLoading: isJoiningGroup }] = useJoinGroupChatMutation()
    const [deleteChat] = useDeleteChatMutation()

    const [activeChat, setActiveChat] = useState<string | null>(contextActiveChat)
